...
> **开发中提示：**项目仍在演进，尚未完工；当前已对 `new_api` 与 `openrouter` 做了官方接口适配（其余平台可能仅支持“手工窗口”兜底展示，具体以实际实现为准）。
...
- **OpenRouter（management key）** - 可配置 management key，支持“测试连接（三级红绿灯）”后保存；展示余额与启用中的 key（`disabled=false`）
...
- **敏感信息**（例如 `new_api` 访问令牌、`openrouter` management key）可存储于 **macOS Keychain**
...
- **添加账号**：菜单选择“添加账号”，按提示选择平台并完成配置
  ...
  - `openrouter`：同一弹窗输入账号名称 / management key；先“测试连接”（三级全绿）再允许保存
...
如曾配置过 `new_api` 令牌或 `openrouter` management key，相关密钥存于 macOS Keychain；建议先在菜单中删除账号以清理 Keychain 条目。
